...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_parse_stdout.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggParseStdout = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_parse_stdout.dart',
      );

      expect(
        ggParseStdout,
        isNotNull,
        reason: 'Item for executing bin/gg_parse_stdout.dart not found',
      );

      expect(ggParseStdout['name'], 'gg_parse_stdout.dart');
      expect(ggParseStdout['type'], 'dart');
      expect(ggParseStdout['request'], 'launch');
